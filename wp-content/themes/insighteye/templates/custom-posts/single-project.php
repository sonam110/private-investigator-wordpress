...
<figure>
	<?php if ( class_exists( 'WPinsighteye_Resizer' ) ) : ?>
		<?php echo wp_kses( $img_obj->WPinsighteye_resize( wp_get_attachment_url( get_post_thumbnail_id( get_the_ID() ), 'full' ), 1100, 600, true ), $allowed_html ); ?>
	<?php else : ?>
...
<div class="blog-detail-meta">
		...
		<?php if ( $options->get( 'projects_single_categories' ) ) : ?>
			<div class="pro-cat">
				<span class="pro-title"><?php esc_html_e('Posted In:', 'insighteye' ); ?></span>
				<span>
				<?php $lists = get_the_terms( get_the_ID(), 'project_cat' ); ?>
					<?php foreach( $lists as $list ) { 
						echo wp_kses(insighteye_set( $list,'name' ), true).', ';
					}
					?>
		...
		<div class="opening-hours">
			...
			<ul>
				...
				<li> 
					...
					<?php echo ( insighteye_set( $entry, 'opening_day') ) ? '<span>'.insighteye_set( $entry, 'opening_day') .':</span>' : ''; ?>

					<?php echo ( insighteye_set( $entry, 'opening_time') ) ? '<p>'.insighteye_set( $entry, 'opening_time') .'</p>' : ''; ?>
				</li>
				...
				<?php if (  $closed = get_post_meta( get_the_ID(), 'project_closed_day', true ) ) : ?>
					<li>
						...
						<i><?php esc_html_e( 'closed', 'insighteye' ); ?></i>
					</li>
				<?php endif; ?>
			</ul>
		</div>
		...
		<?php if ( $options->get( 'projectsingle_post_share' ) && $options->get( 'projectsingle_social_share' ) ) : ?>
			<div class="sharing">
				<span><?php esc_html_e( 'Social Sharing:', 'insighteye' ); ?></span>
					...
					<?php if ( ! empty( $options->get( 'projectsingle_social_share' ) ) ) : ?>
						...
						foreach ( $options->get( 'projectsingle_social_share' ) as $k => $v) {
							if ($v == '')
								continue;
							?>
							<?php do_action('insighteye_social_share_output', $k); ?>
							<?php } ?>

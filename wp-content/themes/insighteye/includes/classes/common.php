...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => insighteye_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => insighteye_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => insighteye_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => insighteye_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => insighteye_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => insighteye_meta( 'banner_elementor_template', $page_id ),
				'banner'            => insighteye_set( insighteye_meta( 'banner_page_background', $page_id ), 'url' ),
				'banner_image'   	=> insighteye_set( insighteye_meta( 'banner_banner_image', $page_id ), 'url' ),
				'title'             => insighteye_meta( 'banner_banner_title', $page_id ),
				'text'             => insighteye_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => insighteye_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : insighteye_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : insighteye_the_title( $template ),
				'banner'        => insighteye_set( $bg, 'url' ),
				'banner_image'  => insighteye_set( $banner_image, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$title = insighteye_meta( 'banner_banner_title', $page_id );
		$text = insighteye_meta( 'banner_banner_text', $page_id );
		$banner_image = insighteye_meta( 'banner_banner_image', $page_id );
		$return = [
